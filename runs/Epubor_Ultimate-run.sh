...
EXE0="EpuborUltimate.exe"
DIR0="$WINEPREFIX/drive_c/Program Files (x86)/Epubor/ultimate"
EXE1="Kindle.exe"
DIR1="$WINEPREFIX/drive_c/Program Files (x86)/Amazon/Kindle"
...
if [ "$Game_Actions" = "Run Epubor Ultimate 3.0.13.812" ]; then
    cd "$DIR0"
    "$W"/bin/wine "$EXE0"
fi
if [ "$Game_Actions" = "Run Kindle" ]; then
    cd "$DIR1"
    "$W"/bin/wine "$EXE1"
fi
