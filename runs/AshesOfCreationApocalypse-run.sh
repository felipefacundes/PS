...
GN=AshesOfCreationApocalypse
SN="Ashes of Creation Apocalypse"
CME="RPG - Ação e aventura - Free to PLay"
...
if [ "$Game_Actions" = "Run ${SN}" ] ; then
    "$W"/bin/wine "$EXE" -gl -applaunch 1133430 2>&1 | tee /dev/stderr | sed -u -n -e '/trace/ s/.*approx //p' | osd_cat --lines=1 --color=yellow --outline=1 --pos=top --align=left
fi
