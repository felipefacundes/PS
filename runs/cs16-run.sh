...
export GN="cs16"
export SN="Counter Strike 1.6"
export CME="Counter-Strike is a first-person shooter where two teams face off to reach a defined goal."
...
EXE0="hl.exe"
DIR0="$WINEPREFIX/drive_c/Program Files (x86)/MONSTERSKILL/Counter-Strike Classic 2019"
...
if [ "$Game_Actions" = "Run ${SN}" ]; then
    cd "$DIR0"
    "$W"/bin/wine "$EXE0" -game cstrike \
    2>&1 | FPS_Xosd
fi
if [ "$Game_Actions" = "Run Half Life" ]; then
    cd "$DIR0"
    "$W"/bin/wine "$EXE0" \
    2>&1 | FPS_Xosd
fi
