...
export GN=CSGlobalOffensiveVK
export SN="Counter-Strike: Global Offensive - Vulkan (DXVK)"
export CME="Counter-Strike: Global Offensive (CS: GO) expands upon the team-based action gameplay."
...
if [ "$Game_Actions" = "Run ${SN}" ]; then
    cd "$DIR0"
    "$W"/bin/wine "$EXE0" -novid -high -nojoy -noforcemaccel -noforcemparms -noforcemspd -nod3d9ex -processheap -lv -tickrate 128 -refresh 120 -applaunch "$Steam_Game_ID" \
    2>&1 | FPS_Xosd
fi
