...
export GN="Crysis"
export SN="Crysis"
export CME="Crysis - Adapt to Survive — An epic story thrusts players into an ever-changing environment"
...
if [ "$Game_Actions" = "Run ${SN}" ]; then
    cd "$DIR0"
    "$W"/bin/wine "$EXE0" "$Pr4" -applaunch "$Steam_Game_ID" \
    2>&1 | FPS_Xosd
fi
if [ "$Game_Actions" = "Run ${SN}" ]; then
    cd "$DIR1"
