...
GN=albion
SN="Albion Online"
CME="Albion Online is the first truly cross-platform MMO experience, available for Windows, Mac, Linux and Android, with all players on one master server in one."
...
if [ "$Game_Actions" = "Run ${SN}" ] ; then
    "$W"/bin/wine "$EXE" -dx11 -applaunch 761890 \
